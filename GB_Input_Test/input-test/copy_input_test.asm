...
STRIDE_LENGTH: DS 1; Holds the value of the x value that the player charcter travels per frame
...
begin: ; GingerBread assumes that the label "begin" is where the game should start
    ...
    ; Set up the value for the stride length
    ld a, 2
    ld [STRIDE_LENGTH], a
    ...
    .if
        ...
        .firstIf
            ...
            .drawCrouch
                ...
                ; If player is in the air, do not draw crouch
                ld a, [PLAYER_POSITION+1]
                cp 128
                jr nz, .firstIf2

                ; If player was running and then pressed down
                ; Have the character slide foward a bit
                ld a, [PLAYER_VELOCITY]
                ; If they just crouched down don't have them slide
                cp 0
                jr z, .continue
                BIT 7, a
                jr z, .posVel
...
; And for adjusting of gloabal variables due to what buttons were pressed
;------------------------------------------------------------------------------------------------
displayStart:
...
displayB:
    ...
    ; Make the wait var less then the default since the b buttin was pressed down
    ;ld a, 6
    ;ld [WAIT_VAR], a
 
    
    ; If player is holding down the down button, don't update velocity
    ld a, [CURRENT_INPUT]
    and KEY_DOWN
    cp 0
    jr nz, .ret
...
.continue
    ...
    ; First check if player is going to the left or right
    ld a, [CURRENT_INPUT]
    and KEY_LEFT
    cp 0
    jr nz, .negVelocity
...
hideB:
    ...
    ; Make the wait var back to its default value since the b buttin was released
    ;ld a, 7
    ;ld [WAIT_VAR], a

    ; If player was not moving, do not update velocity on x axis
    ld a, [PLAYER_VELOCITY]
    cp 0
    jr z, .continue    
...
displayA:
    ...
    ; If player has jumped already and is holding down the a button still, do not jump again
    ld a, [HAS_JUMPED]
    cp 1
    jr z, .ret

    ; Check to see in what point if there jump the player is currently in 
    ld a, [PLAYER_VELOCITY+1]
    cp 0
    jr z, .beginOfJump
    jr .middleOfJump
...
.if2
    ; If the down button was press, reduced velocity until zero
    ld a, [CURRENT_INPUT]
    and KEY_DOWN
    cp 0
    jr nz, .reduceVelocity
    jr .if3
    .reduceVelocity
        ; Make sure player was running before we reduced
        ld a, [PLAYER_VELOCITY]
        cp -2
        jr z, .ret
        cp 0
...
.if3
    ; Update the velocity on the x axis
    ; First check if the B button was pressed, if so check if down was pressed
    ; If so don't update the value
    ld a, [CURRENT_INPUT]
    and KEY_B
    cp 0
    jr nz, .ret
...
hideLeft:
    ...
    ; Update the velocity on the x axis
    ; First check if the right button was pressed, if it was, don't update the velocity
    ld a, [CURRENT_INPUT]
    and KEY_RIGHT
    cp 0
    jr nz, .ret
   
    ; If player is in the air don't reduce velocity
    ld a, [PLAYER_POSITION+1]
    cp 128
    jr nz, .ret
 
    ; Then check if the player was running
    ; If so, gradully reduce speed to walking until you zero it out
    ld a, [PLAYER_VELOCITY]
    cp 0
    jr nz, .secondCond
    jr .ret
...
hideRight:
    ...
    ; Update the velocity on the x axis
    ; First check if the left button was pressed, if it was, don't update the velocity
    ld a, [CURRENT_INPUT]
    and KEY_LEFT
    cp 0
    jr nz, .ret
